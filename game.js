...
class Soundtrack {
  ...
  update() {
    ...
    const chords = this.mode === "boss" ? this.bossChords : this.mode === "battle" ? this.battleChords : this.introChords;
    ...
    while (this.nextLead < now + lookAhead) {
      ...
      const seq = this.mode === "intro" ? [0, 1, 2, 1] : this.mode === "battle" ? [0, 2, 1, 2] : [2, 1, 0, 1];
      const note = chord[seq[this.leadStep % seq.length]] + (this.mode === "boss" ? 12 : 0);
      const when = this.nextLead;
      this.playNote(this.midiToFreq(note + 12), when, this.beat * 0.22, "square", 0.12, 0.002, 0.08);
    ...
    while (this.nextArp < now + lookAhead) {
      ...
      this.playNote(this.midiToFreq(note), when, this.beat * 0.12, "square", this.mode === "boss" ? 0.08 : 0.06, 0.001, 0.04);
    ...
    while (this.nextDrum < now + lookAhead) {
      ...
      if (this.mode !== "intro" && step === 3) this.playTom(when);
...
window.addEventListener("keydown", () => soundtrack.unlockFromGesture(), { once: true });
window.addEventListener("mousedown", () => soundtrack.unlockFromGesture(), { once: true });
...
class Fighter {
  ...
  update(dt, state) {
    ...
    if (this.ai) {
      ...
      if (Math.abs(dx) < 20 && Math.abs(dy) < 10 && this.cooldown <= 0 && this.jumpZ < 1) {
        ...
        target.damage(this.kind === "mike" ? 14 : this.kind === "finalboss" ? 18 : 8, this.facing * 7);
      }
      return;
    }
  ...
  draw(cameraX) {
    ...
    if (this.facing > 0) {
      rect(sx + 6, torsoTop + 2, arm, 3, skin);
      rect(sx - 8, torsoTop + 2, 3, 3, skin);
    } else {
      rect(sx - 6 - arm, torsoTop + 2, arm, 3, skin);
    ...
    rect(sx - 5, sy - 2 + walk * 0.5, 4, 5, "#1c1c1c");
    rect(sx + 1, sy - 2 - walk * 0.5, 4, 5, "#1c1c1c");
...
const introScenes = [
  {
    ...
    draw(t) {
      ...
      drawPhoto(assets.logo, 0, 0, VIRTUAL_W, VIRTUAL_H);
      rect(0, 0, VIRTUAL_W, VIRTUAL_H, "rgba(0,0,0,0.54)");
...
function drawBackground(cameraX, pulse = 0) {
  ...
  grad.addColorStop(0, "#251f4f");
  grad.addColorStop(0.55, "#16203d");
...
function updateLevel(dt) {
  ...
  soundtrack.setMode(state.finalBossSpawned ? "boss" : state.bossSpawned ? "battle" : "intro");

  if (tap("shift", "shift")) {
    switchHero();
  }

  if (tap("j", "j")) player.attack("punch", state.enemies);
  if (tap("k", "k")) player.attack("kick", state.enemies);
  if (tap("l", "l")) player.attack("spin", state.enemies);
  ...
  if (!state.bossSpawned) {
    if (state.spawnTimer <= 0 && aliveEnemies.length < 5 && state.kills < 18) {
      spawnEnemy();
      state.spawnTimer = 0.9 + Math.random() * 1.1;
    }

    if (state.levelElapsed >= 20) {
      spawnBossMike();
    } else if (state.player.x > WORLD_WIDTH - 420 || state.kills >= 18) {
      if (aliveEnemies.length < 2) spawnBossMike();
    }
  }
  ...
  for (const enemy of state.enemies) {
    if (!enemy.alive && !enemy.counted) {
      ...
      if (enemy.kind === "finalboss") state.finalBossDefeated = true;
      if (Math.random() < 0.22) {
        state.powerUps.push({
  ...
  const aliveAfter = state.enemies.filter((e) => e.alive);
  if (state.bossDefeated && !state.finalBossSpawned && aliveAfter.length <= 1) {
    spawnFinalBoss();
  }
...
function drawLevel() {
  ...
  for (const f of all) {
    ...
    if ((f.kind === "mike" || f.kind === "finalboss") && f.alive) {
      const sx = f.x - state.cameraX;
      text(f.kind === "finalboss" ? "MECHA MIKE" : "MIKE", sx, f.y - 32, 7, "#ff9d9d", "center");
    }
  }
...
function drawEndScreen(win) {
  ...
  if (win) {
    ...
    text("MECHA MIKE IS DOWN.", 160, 90, 8, "#f5f5f5", "center");
...
function loop(now) {
  ...
  if (tap("m", "m")) soundtrack.toggle();
  if (state.scene === "intro") soundtrack.setMode("intro");
  if (state.scene === "complete" || state.scene === "gameover") soundtrack.setMode("battle");
  soundtrack.update();
